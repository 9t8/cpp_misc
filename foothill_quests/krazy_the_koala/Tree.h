 ...
 private:
  ...
  struct Node {
    ...
    static bool is_equal(const Node *p1, const Node *p2) {
      if (p1 == nullptr || p2 == nullptr) return p1 == nullptr && p2 == nullptr;
    ...
    const Node &operator=(const Node &that) {
      ...
      if (that._sibling == nullptr) {
        ...
        if (_sibling == nullptr) _sibling = new Node;
      ...
      if (that._child == nullptr) {
        ...
        if (_child == nullptr) _child = new Node;
        *_child = *that._child;
      }
      return *this;
    }
    ...
    Node *insert_sibling(Node *p) {
      Node *last_node = this;
      while (last_node->_sibling != nullptr) last_node = last_node->_sibling;
      return last_node->_sibling = p;
    }

    Node *insert_child(Node *p) {
      if (_child == nullptr) return _child = p;
    ...
    std::string to_string() const {
      ...
      if (_child == nullptr) return result;
