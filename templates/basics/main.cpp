...
#pragma region Instantiation Examples
  ...
  // Causes implicit instantiation of Max<ll>
  ll (*pfn)(ll, ll) = Max;
...
template <ll size> void Print() {
  char buffer[size];
  std::cout << size << std::endl;
}
// Requires size as explicit argument
template <typename T> T Sum(T *parr, ll size) {
  T sum{};
  for (ll i = 0; i < size; ++i) {
    sum += arr[i];
  }
...
template <typename T, ll size> T Sum(T (&arr)[size]) {
  T sum{};
  for (ll i = 0; i < size; ++i) {
    sum += arr[i];
  }
